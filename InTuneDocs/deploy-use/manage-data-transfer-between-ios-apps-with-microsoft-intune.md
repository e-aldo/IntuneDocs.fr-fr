---
title: "Gérer les transferts de données entre applications iOS | Microsoft Docs"
...
Pour vérifier que les applications que vous déployez à l’aide de votre solution de gestion des appareils mobiles tierce sont également associées aux stratégies de protection des applications configurées dans Intune, vous devez configurer le paramètre UPN d’utilisateur comme décrit dans la procédure [Configurer le paramètre UPN d’utilisateur](#configure-user-upn-setting-for-third-party-emm).  Quand les applications sont déployées avec le paramètre UPN d’utilisateur, les stratégies de protection des applications sont appliquées à l’application quand l’utilisateur final se connecte à l’aide de son compte professionnel.
...
Après avoir configuré le paramètre UPN utilisateur, vous devez vérifier que l’application iOS peut recevoir la stratégie de protection des applications Intune et s’y conformer.

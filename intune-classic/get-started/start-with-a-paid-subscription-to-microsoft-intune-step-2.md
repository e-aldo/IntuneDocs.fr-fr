...
4. La boîte de dialogue **Vérifier le domaine** s’ouvre. Elle contient les valeurs permettant de créer l’enregistrement TXT dans votre fournisseur d’hébergement DNS.
    ...
    > [!TIP]
    > Veillez à créer un alias DNS /intune-classic/deploy-use/set-up-windows-phone-management-with-microsoft-intune) quand vous apportez des modifications dans votre fournisseur d’hébergement DNS.

---
title: "Émettre des certificats PKCS avec Intune"
titlesuffix: Azure portal
description: "Installer et configurer Intune Certificate Connector de façon à ce qu’il émette des certificats PKCS sur des appareils gérés par Intune, à partir du service web Symantec PKI Manager"
keywords: 
...
2. Créez un profil de certificat PKCS sur **Microsoft Intune** > **Configuration de l’appareil – Profils** > **Créer un profil** > **Certificat PKCS**.
   ...
   > Les paramètres suivants du profil de certificat PKCS doivent être configurés avec les valeurs spécifiées dans le tableau suivant, comme dans la capture d’écran ci-dessous, pour pouvoir émettre des certificats PKCS par le biais d’Intune Certificate Connector à partir de l’AC Symantec. 
    ...
    | --- | --- | --- |
    | Autorité de certification | pki-ws.symauth.com | Cette valeur doit être l’URL de service de base de l’AC Symantec, sans les barres obliques de fin.  Si vous n’avez pas la certitude qu’il s’agisse de la bonne URL de service de base pour votre abonnement à l’AC Symantec, contactez le service clientèle de Symantec. <br><br> Si cette URL est incorrecte, Intune Certificate Connector n’émettra pas de certificats PKCS à partir de l’AC Symantec.| 
...
| NDES Connector - IssuePfx - Exception générique : <br> System.NullReferenceException : La référence d'objet n'a pas pour valeur une instance d'un objet. | Il s’agit d’une erreur temporaire. Redémarrez Intune Service Connector. <br><br> 1. Ouvrez services.msc. <br> 2. Sélectionnez **Service Intune Connector**. <br> 3. Cliquez avec le bouton droit et sélectionnez **Redémarrer**. |
| Fournisseur Symantec - Impossible de récupérer la stratégie Symantec « L’opération a expiré. » | Intune Certificate Connector a reçu une erreur d’expiration du délai d’attente de l’opération lors de la communication avec l’AC Symantec. Si cette erreur persiste, augmentez la valeur du délai de connexion, puis réessayez. <br><br> Pour augmenter le délai de connexion : <br> 1. Accédez à NDES Connector ordinateur 2. Ouvrez le fichier `%ProgramFiles%\Microsoft Intune\NDESConnectorSvc\NDESConnector.exe.config` dans le Bloc-notes. <br> 3. Augmentez la valeur du délai pour le paramètre suivant : <br><br> `CloudCAConnTimeoutInMilliseconds` <br><br> 4. Redémarrez le service Intune Connector. <br> Si le problème persiste, contactez le service clientèle de Symantec. |
